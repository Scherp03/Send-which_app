...
export const oauth = async (req, res, next) => {
  ...
  try {
    ...
    await CreateAndLoginUser(oAuth2Client.credentials.access_token, res);
  } catch (err) {
...
async function CreateAndLoginUser(access_token, res) {
  ...
  // if it is the first time accessing, create the user and then login
  else {
    const url = 'http://localhost:3000/api/v1/users'; // Replace with your API endpoint
